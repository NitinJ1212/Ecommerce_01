...
export const createDummyProductss = async (req, res) => {
  try {
    const dummyProducts = [
      {
...
export const createDummyProducts = async (req, res) => {
  try {
    ...
    const { name, description, price, category, brand, stock, sub_category } = req.body;
    const data = req.body;
    if (!name || !description || !price || !category || !sub_category || !stock || !brand) {
      return res.status(400).json({ message: "Please fill all required fields" });
    }
